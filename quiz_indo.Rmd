...
```{r}
# your code here
ggcorr(data = crime, label = TRUE,hjust = 1, label_size = 3, layout.exp = 3)
```
```{r}
...
```{r}
# your code here
model_allcrime <- lm(crime_rate~police_exp60+time_prison, data=crime)
...
3. Kesimpulan apa yang paling tepat dari model regresi diatas?
  - [x ] Nilai R-squared mendekati 0.46, sehingga mengindikasikan model yang belum baik (nilai yang mendekati 1 semakin baik)
...
```{r}
model_allcrime <- lm(crime_rate~., data=crime)
...
```{r}
# your code here
step(object = model_allcrime, direction = "backward", trace = T)
```
```{r}
model_step <-lm(formula = crime_rate ~ percent_m + mean_education + police_exp60 + m_per1000f + unemploy_m24 + unemploy_m39 + inequality + prob_prison, 
    data = crime)
summary(model_step)
```
...
```
```{r}
prediction_charges <- predict(object = model_step, newdata = crime_test)
```

```{r}
crime_test %>% 
  mutate(charges = prediction_charges)
```
...
```{r}
# your code here
MAE(y_pred = model_back$fitted.values, y_true = crime_test$crime_rate)
```
___
